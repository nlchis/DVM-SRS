...
| **FR-order-tracking-002** | Phê duyệt đơn hàng thủ công | Admin phê duyệt đơn hàng (Chờ Duyệt -> Đã duyệt) và sinh Phiếu xuất kho. | P0 |
...
| **FR-order-tracking-004** | Đồng bộ hành trình từ 247Express | Tự động nhận Webhook từ 247Express để cập nhật trạng thái: Đang Giao, Đã giao, Thất bại, Đang Hoàn, Đã Hoàn. | P0 |
| **FR-order-tracking-005** | Hoàn hàng do Khách yêu cầu | Cho phép Sales bấm [Hoàn hàng 1 phần] khi đơn đã Đã giao. Validate số lượng trả lại. Tính thêm phí vận chuyển hoàn. | P0 |
...
| **FR-order-tracking-007** | Xuất kho tự động và In chứng từ | Admin duyệt đơn sẽ tự động sinh Bản ghi Xuất kho (Đã duyệt) kèm Phiếu Xuất Kho. Hỗ trợ in Mã vận đơn tại Đơn hàng và in Phiếu Xuất Kho tại Lô xuất. | P0 |
...
| **BR-delivery-001** | Hủy Yêu cầu giao hàng | Chỉ cho phép Admin hủy nếu Yêu cầu chưa có Đơn giao hàng nào, hoặc tất cả các đơn đều đang ở trạng thái Từ chối / Đã hoàn / Đã hủy. Báo lỗi "Yêu cầu hủy đơn giao hàng trước" nếu vi phạm. |
...
| **BR-delivery-003** | Ghi nhận số lượng Yêu cầu | Số lượng "đã giao thực tế" của Yêu cầu giao hàng CHỈ tăng lên khi một Đơn giao hàng đạt trạng thái **Đã giao**. |
| **BR-delivery-004** | Auto Refund Số lượng (Hoàn) | Khi 1 Đơn bị chuyển thành **Từ chối** hoặc **Đã hoàn**, hệ thống lập tức trừ đi số lượng của đơn đó khỏi Yêu cầu giao hàng. Điều này có thể làm Yêu cầu giao hàng bị lùi trạng thái từ Hoàn thành về Chờ xử lý. |
...
| **BR-order-tracking-009** | Khấu trừ tồn kho tự động | Khi Admin duyệt đơn, hệ thống tự động sinh Bản ghi Xuất kho (Đã duyệt), trừ tồn kho thực tế và chuyển đơn sang **Chờ Giao**. |
| **BR-order-tracking-010** | Không tự động cộng tồn kho | Khi Đơn hàng chuyển sang **Đã hoàn**, hệ thống KHÔNG tự động cộng lại tồn kho. Thủ kho phải tự tạo Bản ghi Nhập kho thủ công (Maker/Checker). |
...
| **BR-order-tracking-015** | Hoàn hàng 1 phần | Nút [Hoàn hàng] chỉ được mở (enabled) khi đơn ở trạng thái **Đã giao**. Không tạo đơn hàng mới. Mọi cập nhật trạng thái vòng Hoàn hàng (Đang hoàn -> Đã hoàn) sẽ ghi đè lên đơn hiện tại nhưng lưu giữ song song lịch sử tracking cũ. |
...
| **Chờ Duyệt** | Admin nhấn Phê duyệt (Approve) | **Chờ Giao** | Trừ kho thực tế, sinh Phiếu xuất kho, gọi API 247Express lấy mã vận đơn. |
| **Chờ Giao** | Webhook 247Express nhận hàng | **Đang Giao** | Gửi SMS "Đang giao hàng" cho khách, gửi Telegram cho Sales. |
| **Đang Giao** | Webhook Giao thất bại lần 1 | **Thất bại** | Cập nhật lý do thất bại. |
| **Đang Giao** | Webhook Giao thành công | **Đã giao** | Cộng *Số lượng đã giao* vào Yêu cầu giao hàng. Gửi SMS thành công. |
| **Thất bại** | Webhook Tự động giao lại | **Đang Giao** | Ghi nhận lượt giao lại. |
| **Thất bại** | Webhook Tự động hoàn (quá hạn) | **Đang Hoàn** | Ghi nhận hành trình hoàn về kho. |
| **Đã giao** | Sales bấm [Hoàn hàng 1 phần] | **Đang Hoàn** | Lưu trữ số lượng hoàn, lý do hoàn. |
| **Đang Hoàn** | Webhook Hoàn kho thành công | **Đã Hoàn** | Auto Refund trừ *Số lượng đã giao* của Yêu cầu giao hàng. (Thủ kho tự nhập tồn kho). |
...
| **Chờ xử lý** | Đơn giao hàng của Yêu cầu này đạt Đã giao, khiến (SL đã giao = SL yêu cầu) | **Hoàn thành** |
| **Hoàn thành** | Đơn giao hàng bị Auto Refund (Từ chối/Đã hoàn), khiến (SL đã giao < SL yêu cầu) | **Chờ xử lý** |
...
| :--- | :--- | :--- | :--- |
| **E-delivery-001** | Lỗi Hủy Yêu cầu | Admin bấm hủy nhưng Yêu cầu vẫn còn Đơn giao hàng đang chạy (Chưa Từ chối/Đã hoàn/Hủy). | "Vui lòng yêu cầu hủy/từ chối các đơn giao hàng trực thuộc trước." |
