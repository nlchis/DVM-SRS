...
│                                                            │
│ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐ │
│ │ Đã tiếp nhận  │ │ Đang đi phát │ │Thành Công │ │ Chờ xử lý  │ │ Chờ chuyển hoàn │ │ Đã chuyển hoàn │ │
│ │    12     │ │    32     │ │    96     │ │     2     │ │     3     │ │
│ └───────────┘ └───────────┘ └───────────┘ └───────────┘ └───────────┘ │
│                                                            │
...
**UI-Rules & Đề xuất UX:**
- **Thẻ Thống kê:** Hiển thị màu sắc tương ứng (Xanh dương = Đã tiếp nhận, Vàng = Đang đi phát, Xanh lá = Giao thành công, Đỏ = Thất bại, Đỏ sẫm = Hoàn hàng). Click vào thẻ sẽ điều hướng tới trang Danh sách Đơn hàng với filter tương ứng được tự động áp dụng.
...
│ ├────────────────────────────────────────────────────────┤ │
│ │ 247-00123 | Nguyen Van A    | Đang đi phát   | [Xem] [In] │ │
...
- **Bộ lọc Trạng thái:** Hỗ trợ lọc danh sách đơn theo 5 trạng thái: Tất cả, Đã tiếp nhận, Đang Vận Chuyển, Thành Công, Thất bại, Chờ chuyển hoàn.
...
│                                                            │
│ TÀI LIỆU ĐÍNH KÈM:                                         │
│ File Hóa đơn:  [ Chọn Tệp... ] (Chưa có tệp nào được chọn)   │
│                                                            │
│ [ Hủy Bỏ ]                                [ Xác Nhận Tạo ] │
└────────────────────────────────────────────────────────────┘
```
...
│ ( ) ...   - Đang đi phát hàng (Chờ cập nhật)                  │
│                                                            │
...
- **Tracking Timeline:** Hiển thị dạng Timeline dọc, tick xanh `(v)` cho các bước đã hoàn tất. Bước hiện tại nhấp nháy hoặc làm nổi bật.
  - *Trạng thái Đã tiếp nhận (Pending):* Chỉ hiển thị mốc "Tạo đơn thành công" (tick xanh), còn mốc "Bưu tá đã lấy hàng" và "Đang đi phát hàng" hiển thị dạng chờ `( ) ...` và làm mờ chữ.
- **Log Thông báo:** Liệt kê các trạng thái gửi SMS/Telegram để Admin/Sales dễ dàng đối chiếu nếu khách khiếu nại chưa nhận được tin.
  - *Trạng thái Đã tiếp nhận (Pending):* Chỉ có dòng thông báo SMS (gửi lúc tạo đơn), ẩn dòng thông báo Telegram bắn tới group Sales.
...
│                                                            │
│ TÀI LIỆU ĐÍNH KÈM:                                         │
│ File Hóa đơn:  [ Chọn Tệp... ] (file_hoa_don_qua_lon.pdf)      │
...
## 6. Màn hình Chi tiết Đơn hàng - Thất bại & Xử lý Hoàn Hàng

```text
┌────────────────────────────────────────────────────────────┐
│ CHI TIẾT ĐƠN HÀNG: #247-00125        Trạng thái: CHỜ XỬ LÝ │
├────────────────────────────────────────────────────────────┤
...
**UI-Rules & Đề xuất UX:**
- **Action Banner:** Khi đơn hàng rơi vào trạng thái cảnh báo ("Thất bại", "Chờ hoàn hàng"), Sales/Admin cần có banner hướng dẫn xử lý nổi bật ở đầu trang chi tiết kèm 2 nút hành động nhanh để điều phối (Giao lại / Hoàn hàng).
...
│ Chứng từ Hóa đơn: [Xem_Hoa_Don.pdf]                       │
...
- **Duyệt đơn:** Nhấn [ PHÊ DUYỆT ĐƠN ] sẽ sinh Bản ghi xuất kho (Chờ duyệt), tự động gọi API đẩy 247Express. Việc trừ kho thực tế diễn ra khi bưu tá Đã lấy hàng.
