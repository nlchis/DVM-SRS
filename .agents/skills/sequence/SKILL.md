...
9.7. **Inline Self-Review (CHỈ khi vượt ngưỡng phức tạp)** — nếu fact-list ở bước 2.5 có **≥3 Alternative/Error Flow**, HOẶC diagram có **≥4 participant**, HOẶC **nesting alt/opt ≥2 tầng**, HOẶC có **callback/timeout/webhook**, BẠN hãy tự review ngay lập tức (inline self-review) thay vì gọi subagent:
   - Tự rà soát: Các actor, nhánh error, dead-end có bị thiếu sót so với fact-list không?
   - Có BLOCKING → tự bổ sung actor/nhánh thiếu vào section, verify lại 9.5+9.6, rồi tiếp bước 10.
   - Loop tối đa 2 vòng tự sửa.
...
10. **Output report:**
    ...
    ```
    `{reviewed_note}` = ` | Inline self-reviewed` nếu bước 9.7 đã chạy, else rỗng.
...
- @../_shared/mermaid-verify.mjs (render-verify sau Write — bước 9.5)
