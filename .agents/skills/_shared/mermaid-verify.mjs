...
function findChrome() {
  const cachePath = path.join(os.homedir(), '.cache', 'puppeteer', 'chrome-headless-shell');
  if (fs.existsSync(cachePath)) {
    const versions = fs.readdirSync(cachePath);
    for (const v of versions) {
      if (v.startsWith('win64-')) {
        const exe = path.join(cachePath, v, 'chrome-headless-shell-win64', 'chrome-headless-shell.exe');
...
blocks.forEach((b, i) => {
  ...
  const res = spawnSync(mmdcCmd, ['-i', mmdPath, '-o', outPath, '-s', '2'], {
    encoding: 'utf8',
