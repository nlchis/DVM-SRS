...
> Họ skill D2: `/d2-activity` (flow) · `/d2-erd` (data model) · `/d2-architect` (kiến trúc này). Cả 3 dùng chung `render.sh` ở `.agents/skills/d2-activity/`.
...
- **Render qua `.agents/skills/d2-activity/render.sh`** (dùng chung). KHÔNG gọi d2/Chrome trực tiếp.
...
```bash
.agents/skills/d2-activity/render.sh docs/{feature}/d2-architect/{slug}.d2
