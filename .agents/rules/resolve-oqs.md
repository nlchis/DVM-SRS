...
| `/reverse-doc` | (none — reverse-doc là gốc: OQ suy từ nguồn, không có upstream. Own OQ ở `docs/_reverse/{feature}/reverse-gaps.md`. Selector nhiều feature: `{feature}:OQ-n`.) |
| `/urd` | `docs/{feature}/brainstorms/*.md` + `docs/_reverse/{feature}/reverse-gaps.md` (nếu có) Mục "Open Questions" |
| `/brd` | brainstorm + `_reverse/{feature}/reverse-gaps.md` + `{feature}-urd.md` |
| `/prd-epic` | brainstorm + `_reverse/{feature}/reverse-gaps.md` + `{feature}-urd.md` + `{feature}-brd.md` |
| `/srs` | brainstorm + `_reverse/{feature}/reverse-gaps.md` + `{feature}-urd.md` + `{feature}-brd.md` + `{feature}-prd.md` |

> `docs/_reverse/{feature}/` (spec + gaps) là **upstream tuỳ chọn** của URD/BRD/PRD/SRS — chỉ inherit khi feature đó có bộ reverse (tái lập từ nguồn). Không có thì bỏ qua, không coi là thiếu. `/srs` đặc biệt: đọc `{feature}-reverse-spec.md` làm nguồn + xác nhận từng mệnh đề 🟡/TBD (giữ nhãn confidence).
...
> **Nhiều feature/doc trong 1 lượt** (vd `/reverse-doc` gom N feature, mỗi feature có `OQ-1` riêng) → selector dùng dạng **`{feature}:OQ-n`** (vd `refund-flow:OQ-2`) để không mơ hồ khi `OQ-id` trùng giữa các file.
...
| `/reverse-doc` | (chỉ scan TRONG `docs/_reverse/{feature}/` reverse-spec + reverse-gaps — reverse ghi TÁCH, KHÔNG đè `urd/brd/srs`; không cascade sang doc khác) |
